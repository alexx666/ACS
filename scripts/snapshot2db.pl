...
snapshot2db.pl - Load session metadata from cxtracker into a db
...
=head2 delete_snapshot_table

 Deletes the snapshot table if it exists.

=cut

sub delete_snapshot_table {
   my ($sql, $sth);
   eval{
      $sql = "DROP TABLE IF EXISTS snapshot_session_" . sanitize_table_name($HOSTNAME);
      $sth = $dbh->prepare($sql);
      $sth->execute;
      $sth->finish;
   };
   if ($@) {
      # Failed
      warn "[*] Drop table snapshot failed...\n" if $DEBUG;
      return 1;
   }
   warn "[*] Dropped table snapshot...\n" if $DEBUG;
   return 0;
}
...
# makes a table name, format: session_$HOSTNAME_$DATE
 makes a table name, format: session_$HOSTNAME
...
sub get_table_name {
  ...
  # my $tablename = "session_" . sanitize_table_name($HOSTNAME) . "_" . "$DATE";
   my $tablename = "snapshot_session_" . sanitize_table_name($HOSTNAME); 	 
  return $tablename;
}
